...
export default function InputBox({
  ...
  useEffect(() => {
   ...
   if(isDrawdown === true){ 
      ...
      let height = `${
        filteredOptions.length * 52.77 < 200
    ...
    <div
      className={
        error
          ...
          : `input-container ${customClass} ${isDisabled ? 'disabled' : ''} ${
              isFocused ? 'focused' : ''
    ...
    >
      {isDrawdown ? (
        <>
          {(isFocused || hasValue) && (
            <div
              className={`${isBoxType === "icon" ? "input-headingdropdown" : "input-heading"}  ${isFocused ? 'focused' : ''} ${
                hasValue ? 'hasValue' : ''
              }`}
            >
              {`${label}`}
            ...
            >
              {filteredOptions.length > 0 ? (
                filteredOptions.map((option, index) => (
                  <div
                    className={`dropdown-options-div ${
        ...
        <>
          {(isFocused || hasValue) && (
            <div
              className={`input-heading ${isFocused ? 'focused' : ''} ${
                hasValue ? 'hasValue' : ''
              }`}
            >
              {label}
